...
func main() {
	...
	var vRouterDescriptor fog05sdk.RouterRecord
	json.Unmarshal(data, &vRouterDescriptor)
	...
	fmt.Printf("Press enter to Create router\n")
	bufio.NewReader(os.Stdin).ReadBytes('\n')

	vRouterR, err := api.Network.AddRouter(n1, vRouterDescriptor)
	check(err)
	fmt.Printf("Result of router creation\n%+v\n", vRouterR)
	...
	fmt.Printf("Press enter to Remove Router\n")
	bufio.NewReader(os.Stdin).ReadBytes('\n')

	err = api.Network.RemoveRouter(n1, vRouterR.UUID)
	check(err)
